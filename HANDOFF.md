...
├── README.md
├── .gitignore                                 ← ignores .build/, build/, EncoreiOS/.build_ios/, generated xcodeproj/Info.plist/entitlements
└── Encore/
    ...
    │   │   ├── Models.swift                   ← Track, CardItem, Shelf, CollectionPage, ArtistPage, Lyrics…
    ...
    │   ├── Encore/                            ← macOS SwiftUI app (AppKit)
    │   └── encore-smoke/                      ← CLI: `swift run encore-smoke` hits the LIVE API unauthenticated
    └── EncoreiOS/                             ← iOS app (separate Xcode project, generated by XcodeGen)
        ├── project.yml                        ← XcodeGen spec → EncoreiOS.xcodeproj (GENERATED, gitignored)
        └── Sources/
            ├── AppMain.swift                  ← @main AppDelegate + PhoneSceneDelegate
            ├── MobilePlayer.swift             ← iOS PlayerEngine (UIKit port of macOS engine)
            ├── MobileServices.swift           ← Theme, DiskCache, PageCache, AuthManager, LibraryStore, Nav, Login views
            ├── MobileRoot.swift               ← TabView (Home/Search/Library), MiniPlayer, ArtworkView, route destinations
            ├── MobileScreens.swift            ← Home/Search/Library/Collection/Artist/Browse + SortFilterBar + artist aggregation
            ├── NowPlayingScreen.swift         ← full-screen now playing (Song/Lyrics/Queue)
...
- Sort + filter (CJK-aware), **persisted per collection** (macOS + iOS)
...
- Playback: queue, shuffle, repeat, song radio, **Autoplay** toggle, auto-radio
  when queue ends, sleep timer (time / song-count), session restore (paused)
- Now Playing: immersive, artwork-tinted; **synced lyrics** (multi-provider:
  YT timed → LRCLIB → NetEase → Musixmatch → YT plain → Genius; user-pickable,
  persisted); Up Next queue; video mode (macOS)
...
- iOS app icon; iOS installable on a free Apple ID (see §7)
...
3. Make the change in `EncoreCore` (shared) if it's logic; then wire UI in BOTH
   `Sources/Encore` (macOS) and `EncoreiOS/Sources` (iOS) for parity — Charlie
