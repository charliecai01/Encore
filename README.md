...
- **Native niceties** — media keys, system/lock-screen Now Playing widget, **AirPlay/output-device picker**, **phone-call/Siri interruption handling** (iOS), Space to play/pause, ⌘←/⌘→ prev/next, ⌘R refresh (macOS), dynamic accent colors from artwork
...
└── iOS/                     # iOS app (XcodeGen; project/scheme named EncoreiOS)
    └── Sources/             # MobilePlayer, MobileScreens, NowPlayingScreen…
```
...
podcast playback speed, and phone-call interruption handling. It's a
separate Xcode project generated by XcodeGen (`project.yml`) — **re-run
`xcodegen generate` after adding or removing source files**. See
[HANDOFF.md](HANDOFF.md) §7 for the full iOS build/deploy
