...
final class LibraryStore: ObservableObject {
    ...
    /// Bumped when a background refresh of `allKnownTracks` lands. The cache
    /// itself isn't @Published, so without this the Library ▸ Songs list kept
    /// showing the array it was handed at load and only picked up changes on
    /// the NEXT visit (Charlie asked whether Songs auto-updates, 2026-08-18).
    @Published private(set) var allTracksVersion = 0
    ...
    /// `forceRefresh` bypasses the cache entirely and awaits a live fetch —
    /// what pull-to-refresh needs. Without it, refreshing Library ▸ Songs was
    /// a no-op: the cache was warm and the once-per-session refresh had
    /// already latched, so it handed back the same array every time.
    func allKnownTracks(forceRefresh: Bool = false) async -> [Track] {
        guard AuthManager.shared.isSignedIn else { return [] }
        if forceRefresh {
            let fresh = await fetchAllKnownTracks()
            guard !fresh.isEmpty else { return allTracksCache ?? [] }
